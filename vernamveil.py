...
class VernamVeil:
    ...
    def _generate_bytes(self, length: int, seed: bytes) -> bytes:
        """
        Produces a key stream of the given length using the key generator function.
    ...
    def _xor_with_key(self, data: bytes, seed: bytes, is_encode: bool) -> tuple[bytes, bytes]:
        ...
        """
        # Preallocate memory and create views to avoid copying when slicing
        data_len = len(data)
        processed = bytearray(data_len)
        view_in = memoryview(data)
        view_out = memoryview(processed)
        for start, end in self._generate_chunk_ranges(data_len):
            ...
            for i in range(chunk_len):
                pos = start + i
                view_out[pos] = view_in[pos] ^ keystream[i]

            # Refresh the seed differently for encoding and decoding
            seed = self._refresh_seed(seed, view_in[start:end] if is_encode else view_out[start:end])
...
def generate_secret_fx(n: int, max_weight: int = 10 ** 5,
                       base_modulus: int = 10 ** 9) -> Callable[[int, bytes, int | None], int]:
    """
    ...
    Returns:
        Callable[[int, bytes, int | None], int]: A function that returns pseudo-random integers from polynomial evaluation.
    """
...
""".strip()
    ...
    local_vars = {}
    exec(function_code, {"hashlib": hashlib}, local_vars)
    fx = local_vars["fx"]
