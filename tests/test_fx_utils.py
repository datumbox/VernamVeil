...
class TestFxUtils(unittest.TestCase):
    ...
    def test_generate_polynomial_fx_scalar(self):
        """Test that generate_polynomial_fx returns a valid scalar function."""
        fx = generate_polynomial_fx(5, max_weight=1000, vectorise=False)
        for i in range(10):
    ...
    def test_check_polynomial_fx_sanity_scalar(self):
        """Test check_polynomial_fx_sanity passes for a good scalar polynomial fx."""
        fx = generate_polynomial_fx(4, max_weight=1000, vectorise=False)
        with warnings.catch_warnings(record=True) as w:
            passed = check_fx_sanity(fx, self.seed, self.bound, self.num_samples)
    ...
    def test_load_fx_from_file_scalar(self):
        ...
        fx_obj = generate_polynomial_fx(3, vectorise=False)
